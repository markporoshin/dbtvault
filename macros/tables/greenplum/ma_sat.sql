{%- macro greenplum__ma_sat(src_pk, src_cdk, src_hashdiff, src_payload, src_eff, src_ldts, src_source, source_model) -%}
...
{{ log('source_model: ' + source_model|string, info=True) }}
...
latest_records AS (
    ...
    SELECT inner_mas_ranked.* FROM (
        ...
        ON {{ dbtvault.multikey([src_pk], prefix=['inner_mas_ranked', 'spk'], condition='=') }}
        WHERE inner_mas_ranked.latest_rank = 1
    ) AS mas
),
